...
describe("REQREP #REQREP", function()
	...
	it("creates a REP socket", function()
		...
		assert.is_truthy(rep1, nml.strerror())
		assert.are_not_equal(-1, rep1, nml.strerror())
	end)
	
	it("binds to the REP socket", function()
		assert.are_not_equal(-1, nml.bind(rep1, socket_address), nml.strerror())
	end)

	it("creates a REQ socket", function()
		req1 = nml.socket(sym.AF_SP, sym.NN_REQ)
		assert.is_truthy(req1)
		assert.are_not_equal(-1, req1, nml.strerror())
	end)

	it("connects the REQ socket", function()
		assert.are_not_equal(-1, nml.connect(req1, socket_address), nml.strerror())
	end)

	it("creates a second REQ socket", function()
		req2 = nml.socket(sym.AF_SP, sym.NN_REQ)
		assert.is_truthy(req2, nml.strerror())
		assert.are_not_equal(-1, req2)
	end)

	it("connects the REQ socket", function()
		assert.are_not_equal(-1, nml.connect(req2, socket_address), nml.strerror())
	end)
	...
	describe("test invalid sequence of sends and recvs #invalid", function()
		it("returns EFSM on invalid send", function()
			...
			assert.are_equal(-1, rc, nml.strerror())
	...
	describe("test fair queueing requests", function()
		it("sends to req2", function()
			assert.are_not_same(-1, nml.send(req2, "ABC", sym.NN_DONTWAIT), nml.strerror())
		end)
	...
	it("closes rep1", function()
		assert.are_equal(0, nml.close(rep1), nml.strerror())
	end)

	it("closes req1", function()
		assert.are_equal(0, nml.close(req1), nml.strerror())
	end)

	it("closes req2", function()
		assert.are_equal(0, nml.close(req2), nml.strerror())
	end)
