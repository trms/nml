...
describe("test NN_PAIR #NN_PAIR", function()
	it("creates a first NN_PAIR socket", function()
		...
		assert.is_truthy(sb, nml.strerror())
		assert.are_not_equal(-1, sb, nml.strerror())	
	end)

	it("binds to the socket", function()
		assert.are_not_equal(-1, nml.bind(sb, SOCKET_ADDRESS), nml.strerror())
	end)

	it("creates a second NN_PAIR socket", function()
		sc = nml.socket(AF_SP, NN_PAIR)
		assert.is_truthy(sc, nml.strerror())
		assert.are_not_equal(-1, sc, nml.strerror())
	end)

	it("connects to the socket address", function()
		assert.are_not_equal(-1, nml.connect(sc, SOCKET_ADDRESS), nml.strerror())
	end)
	...
	it("sb receives the message", function()
		...
		assert.are_not_equal(-1, result, nml.strerror())
